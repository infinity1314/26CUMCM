...
def p4_search_stations() -> List[Point]:
    """A certified 25-station triangular layout for directional discovery.
    ...
    contains at least one receiving station.  The order keeps rare directional
    coverage points early while retaining a short complete route.  A
    continuous-cell certificate covers the complete target disk.
    """
    spacing = 989.0
    ...
    return [
        (spacing * (index[0] + index[1] / 2.0) + offset_x,
         row_height * index[1] + offset_y)
...
def write_policy(problem: int, robot_id: str, output: Path) -> None:
    ...
    stations = p3_search_stations() if problem == 3 else p4_search_stations()
    policy = {
        "format": "cumcm2026b-adaptive-policy-v6",
        "problem": problem,
        "robot_id": robot_id,
