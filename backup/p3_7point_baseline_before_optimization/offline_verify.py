...
def verify_directional_discovery_layout():
    ...
    radial_step = 5.0
    angular_step = math.radians(0.25)
    cell_radius = math.sqrt(
        1800.0 ** 2 + 1802.5 ** 2
        - 2.0 * 1800.0 * 1802.5 * math.cos(angular_step / 2.0)
    ) + 0.01
...
def boundary_outward_sources(count=10):
    sources = {}
    for channel in range(1, count + 1):
        angle = 2.0 * math.pi * (channel - 1) / count
...
def main():
    ...
    print(verify_complete_policy(4, boundary_outward_sources(16)))
